...
class huya(BaseAPI):
    ...
    def get_stream_urls(self, **kwargs) -> str:
        title, uname, face_url, keyframe_url = self.get_info()
        
        if title.startswith("【回放】"):  # 判断是否为回放
            logging.warning(f'虎牙 {self.rid} 是回放，不进行下载.')
            return None  # 或 raise ValueError("检测到回放，停止下载")
