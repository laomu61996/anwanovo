...
def list_folders(drive_name, path):
...
def delete_old_folders(drive_name, sub_path):
    """删除指定路径下，日期超过30天的文件夹"""
    folders = list_folders(drive_name, sub_path)
...
for drive_name in drive_names:
    for base_path in base_paths:
        ...
        sub_folders = list_folders(drive_name, base_path)
        for sub_folder in sub_folders:
            full_path = f"{base_path}/{sub_folder.rstrip('/')}"
            print(f"Checking folder: {full_path} on {drive_name}")
            delete_old_folders(drive_name, full_path)

print("定时删除OneDrive网盘已结束.")
